...
class postC
{
    ...
    function afficherpost1(){
        
        $sql="SELECT post.id_post, post.titre, post.image, post.description, post.date, categories.categorie from post inner join categories on post.id=categories.id ";
        $db = config::getConnexion();
        try{
            $liste=$db->query($sql);
            return $liste;
